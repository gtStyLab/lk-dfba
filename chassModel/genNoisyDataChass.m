function genNoisyDataChass(hiResDataFileName,nT,cov,numSets)

    % 1) Load in hiResDataFileName
    hiResData = load(hiResDataFileName);
    
    % Expect one '_' character
    idxEnd = strfind(hiResDataFileName,'_');
    loResDataFileNameStem = sprintf('%s_nT-%03d_cov-%02d_rep-',hiResDataFileName(1:idxEnd(1)-1),nT,cov*100);
