...
errorFileName = 'processedError_hiRes.mat';
dataDir = 'branchData_hiRes';
nTErrorCalc = 1000;
...
for k = 1:length(fileNameList)
    ...
    if exist(fileName,'file')
        ...
        idxList2 = strfind(fileName,'hiRes');
        tabulatedCalc(k).trainingDataName = sprintf('%s.mat',fileName(idxList1(end)+1:idxList2(end)+4));

        tabulatedCalc(k).referenceData = tabulatedCalc(k).trainingDataName;
        refData = load(sprintf('%s/%s',dataDir,tabulatedCalc(k).referenceData));
        ...
        if ~any(strcmp(fileName,excludeList))
            ...
            if ~isempty(strfind(fileName,'fbaRegressionPlusParams')) || ~isempty(strfind(fileName,'fbaRegressionParams')) || ~isempty(strfind(fileName,'fbaGaParams'))
                ...
                while ntSim > 49 && errorCheck
                    ...
                    data.modelStruct.xBounds = [zeros(size(odeConcMatrix,2),1),inf(size(odeConcMatrix,2),1)];
